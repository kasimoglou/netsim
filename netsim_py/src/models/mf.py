...
from models.constraints import is_legal_identifier, ConstraintViolation,\
    LEGAL_IDENTIFIER
from models.forward import ForwardReference, forward_setattr, forward_invoke
from models.validation import Validation
from models.instrument import attribute_descriptor, relationship_descriptor,\
...
class Annotation:
    ...
    def get(cls, obj, default=Ellipsis, **kwargs):
        ...
        If there is no such annotation, then if default is provided, return the default,
...
class Annotatable:
    """An Annotatable object maintains a list of Annotations.
    """
    ...
    def for_object(obj, create=False, throw=False):
        """Return an Annotatable instance for obj.
        
        If obj is an instance of Annotatable, this returns obj itself.
        ...
        then if throw is True a TypeError is raised, else None is returned.
        
        If obj.__annotatable__ exists and points to an instance of Annotatable,
        then obj.__annotatable__ is returned.
        
        If obj.__annotatable__ exists but is not an instance of Annotatable, if throw 
        is True, a TypeError is raised, else None is returned.
        
        If obj.__annotatable__ does not exist, then:
           * if create is True, obj.__annotatable__ is bound to a new Annotatable
           object and this object is returned.
...
def annotations_of(obj, instance_of=Annotation, **kwargs):
    """Query the list of annotations.
       Return every annotation x where isinstance(x,instance_of) and
       x.matches(kwargs)  
    """
    # N.B. instance_of 
...
#
# We must accept as types the following:
# (a) any Python type
# (b) any mf.Class
# (c) any tuple of the above
#
...
def model(cls):
    ...
    (a) extract a model from the class and 
    (b) instrument the class, together with other classes, as needed  
    """
...
def extract_model(cls):
    """Return a mf.Class from a python class.
    """
